...
def update_figure_data(
    ...
    # Safety check for enough traces
    if len(fig_data) < 5:
        # Return unchanged if we don't have enough traces
        return fig_data
    ...
    gene_traces = [t for t in node_traces if hasattr(t, 'name') and t.name and 'Genes' in t.name]
    phenotype_traces = [t for t in node_traces if hasattr(t, 'name') and t.name and 'Phenotypes' in t.name]
    diagnostic_traces = [t for t in node_traces if hasattr(t, 'name') and t.name and 'Diagnostics' in t.name]

    # Find edge traces by looking at all traces
    # A more robust approach than assuming fixed positions
    gene_pheno_edge_trace = None
    pheno_diag_edge_trace = None

    # Look for edge traces in all data
    for trace in fig_data:
        if hasattr(trace, 'name') and trace.name:
            if 'Gene-Phenotype' in trace.name:
                gene_pheno_edge_trace = trace
            elif 'Phenotype-Diagnostic' in trace.name:
                pheno_diag_edge_trace = trace

    # Update gene-phenotype edge visibility if found
    if gene_pheno_edge_trace is not None:
        if "show_gene_pheno_edges" in controls and not controls["show_gene_pheno_edges"]:
            gene_pheno_edge_trace.opacity = 0
            gene_pheno_edge_trace.visible = False
        else:
            gene_pheno_edge_trace.opacity = 0.6  # Default opacity
            gene_pheno_edge_trace.visible = True

    # Update phenotype-diagnostic edge visibility if found
    if pheno_diag_edge_trace is not None:
        if "show_pheno_diag_edges" in controls and not controls["show_pheno_diag_edges"]:
            pheno_diag_edge_trace.opacity = 0
            pheno_diag_edge_trace.visible = False
        else:
            pheno_diag_edge_trace.opacity = 0.5  # Default opacity
...
def update_visualization(
    ...
    try:
        ...
        with col1:
            # For the bucketed approach, we need to update hover for all phenotype traces
            if len(phenotype_names) > 0:
                # For each phenotype trace in the buckets
                for trace in updated_fig.data:
                    # Safe check for trace name and visibility
                    if (hasattr(trace, 'name') and
                        hasattr(trace, 'visible') and
                        trace.name and
                        'Phenotypes' in trace.name and
                        trace.visible != "legendonly"):

                        # Safe check for hovertext attribute
                        if (hasattr(trace, 'hovertext') and
                            trace.hovertext is not None and
                            len(trace.hovertext) > 0):

                            # Update hover text with HPO names
                            new_hover_texts = []
                            for hpo_id in trace.hovertext:
        ...
        with st.container():
            ...
            with visibility_col2:
                st.markdown('<p style="margin: 0; padding: 0;"><b>Show Edges:</b></p>', unsafe_allow_html=True)
